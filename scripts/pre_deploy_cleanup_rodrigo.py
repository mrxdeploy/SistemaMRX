...
def cleanup():
    ...
    try:
        ...
        # Diagnostics
        cursor.execute("SELECT id, nome FROM fornecedores WHERE nome ILIKE '%RODRIGO GASPAR%'")
        fornecedores_found = cursor.fetchall()

        if not fornecedores_found:
            logging.error("Nenhum fornecedor com o nome RODRIGO GASPAR encontrado!")
            conn.rollback()
            return
            
        logging.info(f"Fornecedores RODRIGO GASPAR encontrados: {[(f['id'], f['nome']) for f in fornecedores_found]}")
        
        # Se houver mais de um (talvez testes), pegaremos o id primário ou faremos loop. Vamos iterar sobre todos eles
        for fornecedor in fornecedores_found:
            FORNECEDOR_ID = fornecedor['id']
            logging.info(f"Iniciando transação de limpeza para o fornecedor_id = {FORNECEDOR_ID} ({fornecedor['nome']})...")
            ...
            logging.info(f"Para FORNECEDOR_ID={FORNECEDOR_ID}: {len(lote_ids)} lotes, {len(oc_ids)} OCs, {len(solicitacao_ids)} solicitacoes, {len(os_ids)} OSs identificados para exclusão.")
            ...
            if lote_ids:
                ...
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de residuos")
                
                # LotesSeparacao
                cursor.execute("DELETE FROM lotes_separacao WHERE lote_id = ANY(%s)", (lote_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de lotes_separacao")

                # MovimentacoesEstoque
                cursor.execute("DELETE FROM movimentacoes_estoque WHERE lote_id = ANY(%s)", (lote_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de movimentacoes_estoque")

                # EntradasEstoque
                cursor.execute("DELETE FROM entradas_estoque WHERE lote_id = ANY(%s)", (lote_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de entradas_estoque")

                # InventarioContagens
                cursor.execute("DELETE FROM inventario_contagens WHERE lote_id = ANY(%s)", (lote_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de inventario_contagens")

                # ItensSolicitacao via Lotes
                cursor.execute("DELETE FROM itens_solicitacao WHERE lote_id = ANY(%s)", (lote_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de itens_solicitacao vinculados a lotes")

            # ItensSolicitacao via Solicitacoes
            if solicitacao_ids:
                cursor.execute("DELETE FROM itens_solicitacao WHERE solicitacao_id = ANY(%s)", (solicitacao_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de itens_solicitacao vinculados a solicitacoes")

            # 3. Deletar dependências de Ordens de Servico e OCs
            if os_ids:
                cursor.execute("DELETE FROM rotas_operacionais WHERE os_id = ANY(%s)", (os_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de rotas_operacionais")

                cursor.execute("DELETE FROM gps_logs WHERE os_id = ANY(%s)", (os_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de gps_logs")

                cursor.execute("DELETE FROM conferencias_recebimento WHERE os_id = ANY(%s)", (os_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de conferencias_recebimento (via OS)")
                ...
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de ordens_servico")

            if oc_ids:
                cursor.execute("DELETE FROM conferencias_recebimento WHERE oc_id = ANY(%s)", (oc_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de conferencias_recebimento (via OC)")

                cursor.execute("DELETE FROM auditoria_oc WHERE oc_id = ANY(%s)", (oc_ids,))
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de auditoria_oc")
            ...
            if sublote_ids:
                ...
                if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de sublotes")

            logging.info("Deletando lotes pai...")
            cursor.execute("DELETE FROM lotes WHERE fornecedor_id = %s", (FORNECEDOR_ID,))
            if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de lotes (fornecedor direto)")

            logging.info("Deletando ordens de compra (OCs)...")
            cursor.execute("DELETE FROM ordens_compra WHERE fornecedor_id = %s", (FORNECEDOR_ID,))
            if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de ordens_compra")

            logging.info("Deletando solicitações...")
            cursor.execute("DELETE FROM solicitacoes WHERE fornecedor_id = %s", (FORNECEDOR_ID,))
            if cursor.rowcount > 0: logging.info(f"Deletados {cursor.rowcount} registros de solicitacoes")
